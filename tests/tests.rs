...
use estudos_rust::{identifier, parser_add, parser_binary_expression, ws};
...
mod tests {
    ...
    fn test_invalid_add_expressions() {
        ...
        assert!(parser_binary_expression(" 3 5 ").is_err());

        // Operador inválido: "3 * 5"
        assert!(parser_binary_expression(" 3 * 5 ").is_err());

        // Termo direito faltando: "3 + "
        assert!(parser_binary_expression(" 3 + ").is_err());
    }
}
